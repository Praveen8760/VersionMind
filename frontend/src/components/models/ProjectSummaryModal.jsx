...
// ------------------------------------------------------------
// CLEAN FORMATTING FUNCTION (removes Markdown symbols)
// ------------------------------------------------------------
function cleanSummary(text) {
  ...
  return text
    ...
    .replace(/-\s{2,}/g, "- ")           // normalize bullets
    .replace(/\n{3,}/g, "\n\n")          // limit whitespace
    .trim();
}
...
export default function ProjectSummaryModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const raw = res.data.summary || "No summary available.";
        setSummary(cleanSummary(raw)); // ← CLEANED SUMMARY
      })
      .catch(() => {
  ...
  return createPortal(
    <AnimatePresence>
      ...
      <motion.div
        key="backdrop"
        className="fixed inset-0 z-[9999] bg-black/45 backdrop-blur-2xl flex items-center justify-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      >
        ...
        >
          ...
          {/* CONTENT */}
          <div className="p-6 overflow-y-auto max-h-[62vh] scrollbar-thin scrollbar-thumb-[#2e323a] space-y-4  hover:scrollbar-thumb-[#3a3f48] scrollbar-track-transparent">
            {loading ? (
          ...
          <div className="px-6 py-4 border-t border-[#1d2127]/70 flex justify-end">
            <button
              onClick={onClose}
              className="px-5 py-2 rounded-lg text-sm bg-[#1a1e24] hover:bg-[#22272f] text-gray-200 transition shadow-sm"
            >
              Close
            </button>
          </div>
